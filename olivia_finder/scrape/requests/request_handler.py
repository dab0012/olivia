...
Last Modified:     Friday February 24th 2023 7:22:39 pm
...
class RequestHandler():
    ...
    _instance = None
    PROXYSCRAPE_URL = 'https://api.proxyscrape.com/?request=getproxies&proxytype=http&timeout=10000&country=all&ssl=all&anonymity=all'
    USERAGENTSTRING_URL = 'https://www.useragentstring.com/pages/useragentstring.php?name=All'

    def __new__(cls, max_request=5):
        if cls._instance is None:
            cls._instance = super().__new__(cls)
            cls._instance.proxies = {}                   # Initialize the proxy list
            cls._instance.user_agents = []               # Initialize the user agent list
            cls._instance.max_request = max_request      # Maximum number of requests to be made with the same proxy
            cls._instance.__obtain_proxies()             # Get proxies and user agents
            cls._instance.__obtain_user_agents()
    ...
    def __obtain_proxies(self) -> bool:
        ...
        try:
            proxies = requests.get(self.PROXYSCRAPE_URL).text
    ...
    def __get_next_user_agent(self) -> Dict[str, str]:
        ...
        # Delete the User Agent with 0.1 probability
