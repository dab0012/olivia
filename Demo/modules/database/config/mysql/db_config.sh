...
# To create a user in mysql it is necessary to have administrator permissions
...
DB_APP_USERNAME=                        # Database username for the application
DB_APP_PASSWORD=                        # Database password for the application

# ----------------------

# Check if there is a mysql installation on the system
# If it doesn't exist, end the script with an error message
...
mysql -u $DB_APP_USERNAME -p$DB_APP_PASSWORD $DB_NAME < db_schema.sql > /dev/null 2>&1
print_status
